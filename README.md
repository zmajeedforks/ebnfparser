...
All licenses are MIT license or similar free and non-viral open-source licenses like BSD
...
- `getopt` has modified source from MUSL libc - it has BSD license - it's only used to process commandline options in executables and not in any library
...
Run the converter `ebnfconvert`
```
build/src/ebnf_convert/ebnfconvert data/gqlgrammar.quotedliterals.txt
```
